...
public class PhaseOne {
	public static class Map extends Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String docName = ((FileSplit) context.getInputSplit()).getPath().getName();//docPart[0]; //getting the document number or the document name
			...
			for (int i = 0; i < docPart.length-1; i++) {
				tempStr = docPart[i].replaceAll("\\p{P}", "") +" "+ docPart[i+1].replaceAll("\\p{P}", ""); // removing special character and punctuation from the
																// word
